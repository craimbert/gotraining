...
func (p programmer) develop(system string) {
	fmt.Printf("programmer %s does %s\n", p.name, system)
}
...
func main() {
	...
	admins.pushAdmin(&sysadmin{"sysadmin1"})
	// Push two new programmers onto devs.
	devs.pushDev(&programmer{"dev1"})
	devs.pushDev(&programmer{"dev2"})
	...
	techfirm := &company{admins.popAdmin(), devs.popDev()}
	...
	for _, task := range tasks {
		...
		if task.needsAdmin {
			...
			fmt.Printf("task: %s, need a admin\n", task.system)
